...
bool CSignScan::ProcessFile(const char *filename) 
{
	...
	{
		pList->InsertItem(m_DisplayCount,L"");
		pList->SetItemText(m_DisplayCount,0,temp);
		pList->EnsureVisible(m_DisplayCount, FALSE);
		pList->SetItemText(m_DisplayCount,1,L"ÓÐ");
		m_DisplayCount++;
	}									
	else
	{	 
		...
		pList->SetItemText(m_DisplayCount,0,temp);
		pList->EnsureVisible(m_DisplayCount, FALSE);
		pList->SetItemText(m_DisplayCount,1,L"Ÿo");
		m_DisplayCount++;
	}
	m_nFileCount++;
	delete[] temp;
...
{   
	...
	dllHandle = LoadLibrary(L"wintrust.dll");   
	if (NULL == dllHandle)   
	{   
