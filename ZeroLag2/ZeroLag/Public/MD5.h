...
void MD5_GenerateSinValue()
{
	...
	double iSinIndex;
	double temp ;
	for ( i=1,iSinIndex=1; i <= 64; i++,iSinIndex++)
	{
		temp = sin( iSinIndex ) ;
		temp = ( temp < 0 ) ? ( temp * (-1) ) : temp ;
		uSinValue[i-1] = (UINT)( temp * gene ) ;
	}
}
...
CString MD5_Caculate_String( CString SourceMsg )
{
	...
	if ( MD5_Caculate( &MD5_Value, (byte*)SourceMsg.GetBuffer(SourceMsg.GetLength()), SourceMsg.GetLength()*8 ) )
	{
		...
		for ( int i = 0; i < 16; i++ )
		{
			TempString.Format ( L"%02X", ((byte*)&MD5_Value)[i] ) ;
...
CString MD5_Caculate_File ( CString szFileName )
{
	...
	{
		MD5_Value_String.Format ( L"CreateFile Error ( code:%d )!", GetLastError() );
		return MD5_Value_String;
	}
	...
	if ( hFileMap == NULL )
	{
		MD5_Value_String.Format ( L"CreateFileMapping Error ( code:%d )!", GetLastError() );
		CloseHandle ( hFile ) ;
		return MD5_Value_String ;
	}
	...
	if ( pMapView == NULL )
	{
		MD5_Value_String.Format ( L"MapViewOfFile Error ( code:%d )!", GetLastError() );
