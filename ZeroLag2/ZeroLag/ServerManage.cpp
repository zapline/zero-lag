...
//start== TRUE启动服务
/ start = false停止服务*/
BOOL CServerManage::ChangeServerStatusByname(LPCSTR servername,BOOL start)
{
	if(servername[0] == '\0')
		return FALSE;
	BOOL         hr;
...
/*得到服务的状态*/
BOOL CServerManage::GetServerStatusByname(LPCSTR  servername,SERVICE_STATUS&  ssStatus)
{
	if(servername[0] == '\0')
		return FALSE;
	HRESULT         hr;
