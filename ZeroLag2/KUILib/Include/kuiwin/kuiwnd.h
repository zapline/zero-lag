...
//KUILIB_API KuiWnds* KuiWnds::ms_pInstance = NULL;
//KUILIB_API CRITICAL_SECTION KuiWnds::ms_lockWndMap;
//////////////////////////////////////////////////////////////////////////
...
public:
    ...
    virtual BOOL Load(TiXmlElement* pTiXmlElem)
    {
        ...
        {
            m_strInnerText = CString( CA2W(pTiXmlElem->GetText(), CP_UTF8) );// Bill Fixed. CA2T not work.
            if (!m_strInnerText.IsEmpty())
            {
                ...
                while (TRUE)
                {
                    nSubStringStart = m_strInnerText.Find(_T("%IDS_"), nSubStringEnd);
                    if (-1 == nSubStringStart)
                        break;

                    nSubStringEnd = m_strInnerText.Find(_T('%'), nSubStringStart + 4);
                    if (-1 == nSubStringEnd)
                        break;
                    ...
                    CString strText = KAppRes::Instance().GetString(UnicodeToAnsi(
                        (const wchar_t *)strId));
#else
					CString strText = KAppRes::Instance().GetString( strId.GetString() );
#endif
